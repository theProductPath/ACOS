...
This doc is for agents and humans adding to ACOS itself — new skills, new templates, new conventions. It's distinct from `adopting-acos.md` (still TODO), which is for someone setting up a new company instance.
...
Every change either belongs in **ACOS** (the framework, here at `Products/ACOS/`) or in an **instance** (a specific company's adoption, like `tPPOS/`). Almost every mistake comes from putting a change in the wrong place. The rule:
...
Skills live at `Products/ACOS/framework/skills/<skill-name>/SKILL.md` with supporting references in `<skill-name>/references/`. The naming and structure conventions:
...
Templates live at `Products/ACOS/framework/templates/<template-name>.md`. The conventions:
...
- **Date your checkpoints.** When the framework gains a meaningful change, write a checkpoint in the calling instance's `_progress/` folder (or in `Products/ACOS/_progress/` if you create one). Future agents will thank you.
...
- **Tooling / runners.** ACOS is markdown conventions, not a tool. If you find yourself proposing a JavaScript or Python runner, you're building a *tool that consumes ACOS*, which belongs in its own product folder.
...
- **Marketing copy.** This folder is the operating manual, not the pitch. Adoption messaging lives in `Products/ACOS/docs/adopting-acos.md` (still TODO) or in a dedicated `*-site` folder if ACOS gets one.
